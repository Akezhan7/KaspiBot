...
class EscalationScheduler:
    ...
    async def process_new_sellers(self) -> None:
        ...
        Если в данный момент идёт полный скан — пропускаем цикл. Иначе мы
        можем отправить WARN1 с обрывочным списком товаров: при сканировании
        каждый offer добавляется в воронку по одному, и до конца скана
        список может содержать только 1–2 карточки из реально прилеплённых.
        """
        scanner = getattr(self._engine, "_scanner", None)
        # Используем `is True` — Mock-объекты в тестах truthy, но не идентичны
        # True. Это гарантирует, что реальный ProductScanner с булевым флагом
        # работает корректно, а тестовые моки не блокируют эскалацию.
        if scanner is not None and getattr(scanner, "is_scanning", False) is True:
            logger.info(
                "Эскалация: идёт полный скан, откладываем WARN1 до его "
                "завершения (иначе список товаров будет неполным)"
            )
            return
